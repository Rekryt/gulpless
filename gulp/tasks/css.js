...
module.exports = function () {
		...
		if (src.length > 0)
			return (
				$.gulp
					...
					.pipe($._if('**/*.less', $.less({ compress: false }), $._if('**/*.scss', $.sass())))
					...
					.pipe($.concat(concat))
					.pipe(
						$._if(!$.production, $.sourcemaps.write('.', { includeContent: true, sourceRoot: sourceRoot }))
					)
					.pipe($.gulp.dest(dest))
	...
	$.gulp.task('_css', function (done) {
		if ($.config._css.src.length > 0) {
			return build_css($.config._css.src, $.config._css.build, './build/', '/_css/');
		} else {
			done();
		}
	});
	...
	$.gulp.task('css', function (done) {
		if ($.config.css.src.length > 0) {
			return build_css($.config.css.src, $.config.css.concat, $.config.css.dest, '/css/');
		} else {
			done();
		}
	});
